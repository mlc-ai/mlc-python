...
if TYPE_CHECKING:
    from mlc._cython import TypeField, TypeInfo
...
def attach_field(
    ...
    old_field = getattr(cls, name, _MISSING)
...
def method_init(
    ...
    for i, field in enumerate(fields):
        ...
        if field.default_factory is _MISSING:
...
def inspect_dataclass_fields(
    ...
    for type_field in type_fields:
        ...
        if lhs.startswith("_"):
            continue
        rhs = getattr(type_cls, lhs, _MISSING)
        if isinstance(rhs, property):
            rhs = type_cls._mlc_dataclass_fields.get(lhs, _MISSING)  # type: ignore[attr-defined]
        if rhs is _MISSING:
            d_field = field()
