...
#define MLC_REGISTER_FUNC(name)                                                                                        \
  [[maybe_unused]] static auto MLC_UNIQUE_ID() = ::mlc::ffi::details::FuncRegistryHelper(name)
...
struct FuncObj : public MLCFunc {
  ...
  template <typename... Args> MLC_INLINE Any operator()(Args &&...args) const {
    ...
    details::FuncCall(this, N, stack_args.v, &ret);
    return ret;
  }
...
protected:
  ...
  MLC_DEF_STATIC_TYPE(FuncObj, Object, MLCTypeIndex::kMLCFunc, "object.Func")
      .Method("__init__", details::ReflectionHelper::DefaultStrMethod);
};

struct FuncObj::Allocator {
public:
  template <typename FuncType, typename = std::enable_if_t<details::HasFuncTraits<FuncType>>>
...
MLC_INLINE void FuncCall(const void *self, int32_t num_args, const MLCAny *args, MLCAny *ret) {
  ...
  if (func->call && reinterpret_cast<void *>(func->safe_call) == reinterpret_cast<void *>(FuncObj::SafeCallImpl)) {
    ...
    HandleSafeCallError(err_code, ret);
  }
}
} // namespace details
} // namespace ffi
