...
inline void PrintAnyToStream(std::ostream &os, const MLCAny *v) {
  MLCVTableHandle vtable;
  Any any_str_func;
  MLCVTableGetGlobal(nullptr, "__str__", &vtable);
  MLCVTableGetFunc(vtable, v->type_index, true, &any_str_func);
  if (::mlc::base::IsTypeIndexNone(any_str_func.type_index)) {
    MLC_THROW(InternalError) << "Method `__str__` is not defined for type "
                             << ::mlc::base::TypeIndex2TypeKey(v->type_index);
  }
  Any ret;
  ::mlc::base::FuncCall(any_str_func.v.v_obj, 1, v, &ret);
