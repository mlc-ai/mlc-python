...
public:                                                                                                                \
  ...
  template <typename DerivedType> MLC_INLINE DerivedType *TryCast() {                                                  \
    if (IsInstance<DerivedType>()) {                                                                                   \
      return reinterpret_cast<DerivedType *>(this);                                                                    \
    } else {                                                                                                           \
      return nullptr;                                                                                                  \
    }                                                                                                                  \
  }                                                                                                                    \
  template <typename DerivedType> MLC_INLINE const DerivedType *TryCast() const {                                      \
    if (IsInstance<DerivedType>()) {                                                                                   \
  ...
  template <typename DerivedType> MLC_INLINE DerivedType *Cast() {                                                     \
    if (IsInstance<DerivedType>()) {                                                                                   \
      return reinterpret_cast<DerivedType *>(this);                                                                    \
    }                                                                                                                  \
    MLC_THROW(TypeError) << "Cannot cast from type `" << ::mlc::base::Type2Str<SelfType>::Run() << "` to type `"       \
                         << ::mlc::base::Type2Str<DerivedType>::Run() << "`";                                          \
    MLC_UNREACHABLE();                                                                                                 \
  }                                                                                                                    \
  template <typename DerivedType> MLC_INLINE const DerivedType *Cast() const {                                         \
    if (IsInstance<DerivedType>()) {                                                                                   \
      return reinterpret_cast<const DerivedType *>(this);                                                              \
    }                                                                                                                  \
    MLC_THROW(TypeError) << "Cannot cast from type `" << ::mlc::base::Type2Str<SelfType>::Run() << "` to type `"       \
...
public:                                                                                                                \
  ...
  MLC_INLINE explicit SelfType(U *src) : ParentType(::mlc::Null) {                                                     \
    this->_SetObjPtr(const_cast<MLCAny *>(reinterpret_cast<const MLCAny *>(src)));                                     \
    this->CheckNull();                                                                                                 \
    this->IncRef();                                                                                                    \
  }                                                                                                                    \
  template <typename U, typename = Derived<U>> /**/                                                                    \
  MLC_INLINE explicit SelfType(const U *src) : ParentType(::mlc::Null) {                                               \
    this->_SetObjPtr(const_cast<MLCAny *>(reinterpret_cast<const MLCAny *>(src)));                                     \
    this->CheckNull();                                                                                                 \
    this->IncRef();                                                                                                    \
  }                                                                                                                    \
  ...
  template <typename... Args,                                                                                          \
            typename = std::enable_if_t<::mlc::base::Newable<ObjType, Args...> && !::mlc::base::IsAny<Args...>>>       \
...
struct Object {
  ...
  std::string __str__() const {
    ...
    os << ::mlc::base::TypeIndex2TypeKey(this->_mlc_header.type_index) << "@0x" << std::setfill('0') << std::setw(12)
       << std::hex << (uintptr_t)(this->_mlc_header.v.v_ptr);
    return os.str();
  }
