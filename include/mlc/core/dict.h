...
template <typename K, typename V> struct DictObj : protected UDictObj {
  ...
  MLC_INLINE const V at(const K &key) const { return UDictObj::at(key).template Cast<V>(); }
  MLC_INLINE const V operator[](const K &key) const { return UDictObj::operator[](key).template Cast<V>(); }
...
public:
  ...
  inline Iterator(UDictObj::Iterator src) {
