...
template <typename _T, typename> MLC_INLINE_NO_MSVC _T AnyView::Cast() const {
  using namespace ::mlc::base;
  using T = RemoveCR<_T>;
  if constexpr (std::is_same_v<T, Any> || std::is_same_v<T, AnyView>) {
    return *this;
  } else if constexpr (IsPOD<T> || IsRawObjPtr<T>) {
...
template <typename _T, typename> MLC_INLINE_NO_MSVC _T Any::Cast() const {
  using namespace ::mlc::base;
  using T = RemoveCR<_T>;
  if constexpr (std::is_same_v<T, Any> || std::is_same_v<T, AnyView>) {
    return *this;
...
template <typename _T, typename> MLC_INLINE_NO_MSVC _T AnyView::CastWithStorage(Any *storage) const {
  using namespace ::mlc::base;
  using T = RemoveCR<_T>;
  MLC_TRY_CONVERT(TypeTraits<T>::AnyToTypeWithStorage(this, storage), this->type_index, Type2Str<T>::Run());
}

template <typename _T, typename> MLC_INLINE_NO_MSVC _T Any::CastWithStorage(Any *storage) const {
  using namespace ::mlc::base;
  using T = RemoveCR<_T>;
  MLC_TRY_CONVERT(TypeTraits<T>::AnyToTypeWithStorage(this, storage), this->type_index, Type2Str<T>::Run());
}
