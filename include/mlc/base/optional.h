...
template <typename T> struct Optional {
  ...
  MLC_INLINE const TObj *operator->() const {
    if (const TObj *ret = ref_.operator->()) {
      return ret;
    }
  ...
  template <typename DerivedType> MLC_INLINE DerivedType *TryCast() {
    if (TObj *obj = ref_.get()) {
      return obj->template TryCast<DerivedType>();
    } else {
      return nullptr;
    }
  }
  template <typename DerivedType> MLC_INLINE const DerivedType *TryCast() const {
    if (const TObj *obj = ref_.get()) {
      return obj->template TryCast<DerivedType>();
    } else {
      return nullptr;
    }
  ...
  template <typename DerivedType> MLC_INLINE DerivedType *Cast() {
    if (TObj *obj = ref_.get()) {
      return obj->template Cast<DerivedType>();
    } else {
      return nullptr;
    }
  }
  ...
  MLC_INLINE bool operator!=(std::nullptr_t) const { return defined(); }
  // TODO: Do we need `operator bool()` at all?
