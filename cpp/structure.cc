...
Any JSONLoads(AnyView json_str) {
  if (json_str.type_index == kMLCRawStr) {
    return ::mlc::JSONLoads(json_str.operator const char *());
  } else {
    return ::mlc::JSONLoads(json_str.operator Str());
  }
}

Any JSONDeserialize(AnyView json_str) {
  if (json_str.type_index == kMLCRawStr) {
    return ::mlc::Deserialize(json_str.operator const char *());
  } else {
    return ::mlc::Deserialize(json_str.operator ::mlc::Str());
  }
}
