...
MLC_API int32_t MLCTypeDefReflection(MLCTypeTableHandle _self, int32_t type_index, int64_t num_fields,
  ...
  MLC_SAFE_CALL_BEGIN();
  TypeTable *self = TypeTable::Get(_self);
  auto *type_info = self->GetTypeInfoWrapper(type_index);
  type_info->SetFields(num_fields, fields);
  type_info->SetMethods(num_methods, methods, [self, type_index](const char *name, FuncObj *func) {
    ::mlc::registry::VTable *vtable = self->GetGlobalVTable(name);
    vtable->Set(type_index, func, 0);
  });
...
MLC_API int32_t MLCVTableGetFunc(MLCVTableHandle vtable, int32_t type_index, bool allow_ancestor, MLCAny *ret) {
...
MLC_API int32_t MLCVTableCall(MLCVTableHandle vtable, int32_t num_args, MLCAny *args, MLCAny *ret) {
  using ::mlc::registry::VTable;
  MLC_SAFE_CALL_BEGIN();
  static_cast<VTable *>(vtable)->Call(num_args, args, ret);
  MLC_SAFE_CALL_END(&last_error);
}

MLC_API int32_t MLCVTableStaticCall(MLCVTableHandle vtable, int32_t type_index, int32_t num_args, MLCAny *args,
                                    MLCAny *ret) {
  using ::mlc::registry::VTable;
  MLC_SAFE_CALL_BEGIN();
  static_cast<VTable *>(vtable)->StaticCall(type_index, num_args, args, ret);
  MLC_SAFE_CALL_END(&last_error);
}
