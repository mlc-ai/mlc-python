...
struct VTable {
  ...
  int32_t Call(int32_t num_args, const MLCAny *args, MLCAny *ret);
  int32_t StaticCall(int32_t type_index, int32_t num_args, const MLCAny *args, MLCAny *ret);
...
struct TypeInfoWrapper {
  ...
  void ResetMethods() {
    if (this->num_methods > 0) {
      ...
      for (int32_t i = 0; i < this->num_methods; i++) {
        MLCFunc **func = &methods[i].func;
        this->pool->DelPODArray(methods[i].name);
        this->pool->DelObj(*func);
        *func = nullptr;
      }
      this->pool->DelPODArray(methods);
  ...
  void SetMethods(int64_t new_num_methods, MLCTypeMethod *methods,
                  std::function<void(const char *, FuncObj *)> set_vtable) {
    this->ResetMethods();
    this->num_methods = new_num_methods;
    MLCTypeMethod *dsts = this->info.methods = this->pool->NewPODArray<MLCTypeMethod>(new_num_methods + 1);
    for (int64_t i = 0; i < num_methods; i++) {
      MLCTypeMethod &dst = dsts[i] = methods[i];
      FuncObj *func = reinterpret_cast<FuncObj *>(dst.func);
      this->pool->AddObj(func);
      dst.name = this->pool->NewStr(dst.name);
      set_vtable(dst.name, func);
      // {
      //   std::unordered_map<int32_t, FuncObj *> &vtable = (*vtables)[dst.name];
      //   auto [it, success] = vtable.try_emplace(type_index, nullptr);
      //   if (!success) {
      //     // TODO: decide whether to allow overriding
      //   } else {
      //     it->second = func;
      //     this->pool->AddObj(func);
      //   }
    ...
    dsts[num_methods] = MLCTypeMethod{};
    std::sort(dsts, dsts + num_methods,
...
inline int32_t VTable::Call(int32_t num_args, const MLCAny *args, MLCAny *ret) {
  FuncObj *func = this->GetFunc(args[0].type_index, true);
  ::mlc::base::FuncCall(func, num_args, args, ret);
  return 0;
}

inline int32_t VTable::StaticCall(int32_t type_index, int32_t num_args, const MLCAny *args, MLCAny *ret) {
  FuncObj *func = this->GetFunc(type_index, true);
  ::mlc::base::FuncCall(func, num_args, args, ret);
  return 0;
}
